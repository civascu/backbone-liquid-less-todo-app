...
(function () {
		...
		trigger: function (events) {
			...
			while (node = events.pop()) {
				...
				while ((node = node.next) !== tail) {
					//"false" returned from an "event handler" stops further event "handling"
					var continueProcessing = node.callback.apply(node.context || this, args);
	...
	_.extend(Backbone.History.prototype, Backbone.Events, {
		...
		navigate: function (fragment, options) {
			...
			//don't navigate if there is a beforeroute event handler that returns false
			var continueRoute = this.trigger('beforeroute', this);
			if (continueRoute === false) {
				return;
			}

			// If pushState is available, we use it to set the fragment as a real URL.
			if (this._hasPushState) {
				if (!frag.indexOf(this.options.root)) frag = frag.substr(this.options.root.length);
